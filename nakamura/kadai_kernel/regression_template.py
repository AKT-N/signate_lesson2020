...
class linearRegression():
	...
	# y: �o�̓f�[�^�i�f�[�^���j
	def loss(self,x,y):
		loss = np.square(y - self.predict(x)).mean(axis=0)
		return loss
	#------------------------------------
	...
	def calcDist(self,x,z):
		X = np.tile(np.expand_dims(x,axis=2),[1,1,z.shape[1]])
		Z = np.tile(np.expand_dims(z,axis=1),[1,x.shape[1],1])
